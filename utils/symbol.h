/*
 * symbol management data structures for motrace
 *
...
#ifndef MOTRACE_SYMBOL_H
#define MOTRACE_SYMBOL_H
...
#define MOTRACE_PYTHON_MODULE_NAME "motrace_python"
#define MOTRACE_PYTHON_SYMTAB_NAME "python.fake"

enum motrace_symtype {
	ST_UNKNOWN = '?',
	ST_LOCAL_FUNC = 't',
	ST_GLOBAL_FUNC = 'T',
	ST_WEAK_FUNC = 'w',
...
struct motrace_symtab {
	 ...
	 */
	struct motrace_symbol **sym_names;
...
static inline bool is_kernel_address(struct motrace_sym_info *sinfo, uint64_t addr)
{
	return addr >= sinfo->kernel_base;
}

/* convert rstack->addr (or rec->addr) to full 64-bit address */
static inline uint64_t get_kernel_address(struct motrace_sym_info *sinfo, uint64_t addr)
{
	return addr | sinfo->kernel_base;
}
...
extern struct motrace_symbol sched_sym;
extern struct motrace_symbol sched_preempt_sym;
...
#define MAP_KERNEL (struct motrace_mmap *)1
...
int load_symbol_file(struct motrace_sym_info *sinfo, const char *symfile, uint64_t offset);
void save_symbol_file(struct motrace_sym_info *sinfo, const char *dirname, const char *exename);
...
void setup_skip_idx(struct motrace_sym_info *sinfo);
